...
load("data/liiklusonnetused_kauplused.RData")
```

Tutvu kaupluste andmetega
```{r}
glimpse(kauplused_koordinaatidega)
```
...
Töötle kaupluste andmeid selliselt, et alles jäävad ainult täpse koordinaadi saanud poed
```{r}
kauplused <- kauplused_koordinaatidega %>% 
  filter(loctype == "rooftop") %>% 
  arrange(lon, lat, desc(kaubanduskeskus)) %>% 
  distinct(lon, lat, .keep_all = TRUE) %>% 
  select(-type, -loctype, -north:-west) %>% 
  filter(kaubanduskeskus == 1)
```
...
```{r}
kaubanduskeskuse_buffer <- kauplused_sf %>% 
  st_buffer(dist = 250) 
# 
